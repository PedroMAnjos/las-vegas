...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // SERVIÇO DE AUTENTICAÇÃO E SPA GUARD (SUPABASE)
    // ==========================================
    ...
    const AuthService = {
        ...
        async login(username, password) {
            ...
            if (data.requireHash) {
                localStorage.setItem('sysTempToken', data.tempToken);
                return { role: data.role, requireHash: true };
            }
            
            localStorage.setItem('sysToken', data.token);
            localStorage.setItem('sysSession', JSON.stringify(data.session));
            return data.session;
        },
    ...
    if (btnVerifyOp) {
        btnVerifyOp.addEventListener('click', async () => {
            ...
            try {
                const tempToken = localStorage.getItem('sysTempToken');
                
                if (!tempToken) throw new Error("Sessão inválida ou expirada. Faça login novamente.");
                
                // Fluxo exclusivo via Backend
    ...
    function logAction(action, details) {
        ...
        // Envia o Log para o Banco de Dados (Supabase)
        ...
        if (token) {
            ...
            }).catch(e => console.error("[LOG SYNC] Erro ao enviar log pro DB:", e));
        }
    ...
    async function loadData() {
        try {
            ...
            if (token) {
                ...
                // Puxa os Logs do Banco de Dados
                try {
                    ...
                    if (resLogs.ok) {
                        ...
                        systemLogs = logsData.logs.map(l => {
                            ...
                            return {
                                ...
                                operator: l.operator_name || 'Sistema',
                                action: l.action,
                                details: l.details
                            };
                        });
    ...
    async function saveData() {
        try {
            ...
            if (currentSession.role === 'admin') {
                if (token) {
                    try {
                        const resFin = await fetch('/api/finance', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                            body: JSON.stringify({ transactions })
                        });
                        if (!resFin.ok) console.error("[SYNC] Erro no backend financeiro ao salvar.");
                    } catch (e) {
                        console.error("[SYNC] Erro de rede ao conectar com backend financeiro:", e);
                    }
                }
    ...
    function renderFinance() {
        ...
        document.querySelectorAll('.btn-del-trans').forEach(b => b.onclick = () => removeTransaction(b.dataset.id));
    }
    ...
    // ==========================================
    // MODAIS, MÁSCARAS E FORMULÁRIO MANUAL
    // ==========================================
